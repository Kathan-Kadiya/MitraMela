...
export async function signup(req,res){
    ...
    try{
        ...
        const idx = Math.floor(Math.random()*100) + 1; // generates random avatars from ids 1-100
    ...
    try {
        ...
        if(!updatedUser){
            return res.status(404).json({
                message: "User not found in onboard function."
            });
        }
        ...
        try {
            ...
            console.log("Error in the stream onboarding.",error);
        }

        res.status(200).json({success : true, user : updatedUser});

    } catch (error) {
        console.log("Error in onboarding the user.");
        res.status(500).json({
